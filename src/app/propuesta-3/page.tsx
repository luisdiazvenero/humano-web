...
import { IntentId } from "@/components/humano/IntentSelector"
...
export default function Propuesta3() {
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-background text-foreground relative overflow-hidden selection:bg-primary selection:text-primary-foreground">
      ...
      {/* FULL WIDTH HEADER - Eddie.eco style */}
      <header className="relative z-10 w-full px-6 lg:px-12 py-6 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
      ...
      {/* SPLIT SCREEN HERO - Eddie.eco style */}
      <section className="relative z-10 w-full h-screen grid lg:grid-cols-2">
        {/* LEFT HALF - Text Content */}
        <div className="flex flex-col items-center justify-center px-6 lg:px-12 py-16 bg-background">
          <div className="max-w-xl w-full space-y-8 text-center animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
            {/* Main Heading */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif leading-tight text-foreground">
              Hola, soy tu anfitrión en Humano, Miraflores.
            </h1>

            {/* Subtitle */}
            <p className="text-xl sm:text-2xl text-muted-foreground">
              ¿Vienes por trabajo, descanso o aventura?
            </p>

            {/* Intent Buttons - Horizontal */}
            <div className="flex flex-wrap gap-3 justify-center pt-4">
              <button
                onClick={() => handleIntentSelect("work")}
                className={`px-6 py-3 rounded-full text-sm font-medium transition-all duration-300 border-2 ${intent === "work"
                    ? "bg-primary text-primary-foreground border-primary"
                    : "bg-card/60 border-border/50 hover:bg-primary/20 hover:border-primary"
                  }`}
              >
                💼 Trabajo
              </button>
              <button
                onClick={() => handleIntentSelect("rest")}
                className={`px-6 py-3 rounded-full text-sm font-medium transition-all duration-300 border-2 ${intent === "rest"
                    ? "bg-primary text-primary-foreground border-primary"
                    : "bg-card/60 border-border/50 hover:bg-primary/20 hover:border-primary"
                  }`}
              >
              ...
              <button
                onClick={() => handleIntentSelect("adventure")}
                className={`px-6 py-3 rounded-full text-sm font-medium transition-all duration-300 border-2 ${intent === "adventure"
                    ? "bg-primary text-primary-foreground border-primary"
                    : "bg-card/60 border-border/50 hover:bg-primary/20 hover:border-primary"
                  }`}
              >
        ...
        <div className="relative overflow-hidden bg-black">
          ...
          <div className="absolute inset-0 flex items-center justify-center z-10">
            <div className="text-center space-y-4 opacity-0 hover:opacity-100 transition-opacity duration-300">
              <div className="h-16 w-16 rounded-full bg-white/10 backdrop-blur-sm border-2 border-white/50 flex items-center justify-center mx-auto cursor-pointer hover:scale-110 transition-transform">
      ...
      {/* PREVIOUS HERO CONTENT - Moved Below */}
      <section className="relative z-10 mb-16 px-6 lg:px-12 py-16 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
