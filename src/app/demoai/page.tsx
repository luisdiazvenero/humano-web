...
export default function DemoPage() {
  ...
  const getSuggestionIcon = (suggestion: string) => {
    ...
    // Main intents
    if (lower.includes('trabajo')) return <Briefcase className="h-5 w-5" />
    if (lower.includes('descanso')) return <Palmtree className="h-5 w-5" />
    if (lower.includes('aventura')) return <Compass className="h-5 w-5" />
    // Free text
    if (lower.includes('cuéntame') || lower.includes('libremente')) return <ChatBubbleLeftIcon className="h-5 w-5" />
    // Grupo selection
    if (lower.includes('solo')) return <UserIcon className="h-5 w-5" />
    if (lower.includes('pareja')) return <UsersIcon className="h-5 w-5" />
    if (lower.includes('grupo') && !lower.includes('cotización') && !lower.includes('actividad')) return <UserGroupIcon className="h-5 w-5" />
    // Specific actions
    if (lower.includes('cotización')) return <CurrencyDollarIcon className="h-5 w-5" />
    if (lower.includes('early') || lower.includes('check in')) return <ClockIcon className="h-5 w-5" />
    if (lower.includes('contacto')) return <PhoneIcon className="h-5 w-5" />
    if (lower.includes('sala')) return <PresentationChartBarIcon className="h-5 w-5" />
    if (lower.includes('actividad')) return <SparklesIcon className="h-5 w-5" />
    if (lower.includes('restaurante')) return <BuildingStorefrontIcon className="h-5 w-5" />
    // General suggestions
    ...
    if (lower.includes('bar')) return <BuildingStorefrontIcon className="h-5 w-5" />
    ...
    if (lower.includes('asesor') || lower.includes('hablar')) return <PhoneIcon className="h-5 w-5" />
    return <ChatBubbleLeftIcon className="h-5 w-5" />
  }

  // Get description for suggestion based on text
  const getSuggestionDescription = (suggestion: string) => {
    const lower = suggestion.toLowerCase()
    if (lower.includes('trabajo')) return 'Wifi rápido, reuniones y foco'
    if (lower.includes('descanso')) return 'Relax, silencio y confort'
    if (lower.includes('aventura')) return 'Explorar Lima como local'
    return null
  }
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    if (!selectedCaracteristica && (lower.includes('trabajo') || lower.includes('descanso') || lower.includes('aventura'))) {
      ...
      if (lower.includes('aventura')) caracteristica = 'aventura'
