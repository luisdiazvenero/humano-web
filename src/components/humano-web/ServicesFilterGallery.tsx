...
export function ServicesFilterGallery({ services }: { services: HumanoService[] }) {
  ...
  const filteredServices = services.filter((service) => {
    const matchesIntent =
      !selectedIntent ||
      service.intenciones.map(normalizeValue).includes(normalizeValue(selectedIntent))
    const matchesProfile =
      !selectedProfile ||
      service.perfilIdeal.map(normalizeValue).includes(normalizeValue(selectedProfile))
          ...
          {filteredServices.length === 0 ? (
            <div className="rounded-[32px] border border-[var(--color-azul-rgb)]/10 bg-white/60 px-8 py-14 text-center shadow-[0_10px_28px_rgba(0,48,53,0.05)]">
              ...
              <p className="mx-auto mt-3 max-w-[34ch] text-[17px] leading-relaxed text-[var(--color-azul-rgb)]">
                Prueba otra combinación y te mostramos servicios más cercanos a ese plan.
              </p>
            </div>
          ) : (
